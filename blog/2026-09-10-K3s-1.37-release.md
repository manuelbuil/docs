...
In K3s v1.37, we have prioritized simplifying production-grade cluster operations, hardening security perimeters, and dramatically modernizing our testing ecosystem. Whether you are running K3s on a handful of IoT devices or orchestrating complex High Availability topologies, here is a deep dive into what is new in K3s v1.37!
...
### Storage Revolution: Kine SQL Performance & The All-New T4 Backend 🚀
For clusters running on relational database backends (like SQLite, PostgreSQL, and MySQL), Kine is the engine translating etcd calls into SQL queries. This release introduces support for Kine's brand-new T4 storage driver (`t4://`), an open-source key-value database built directly on S3-compatible object storage (like MinIO, Ceph, or GCS). Pointing Kine at an S3 bucket gives you a highly available, clustered control plane with zero Raft quorum complexity. Additionally, core SQL query performance has been optimized by up to 20% by reducing subqueries and leveraging prepared statements for watch polling, while default idle connections have been hardened to prevent database connection thrashing.

### ServiceLB Multi-Pools (Load Balancer Node Pools) 🏗️
ServiceLB, our built-in lightweight load balancer controller, has received a massive architectural upgrade to support **multiple isolated load-balancer pools**. Utilizing node affinity, K3s can now restrict its ServiceLB DaemonSets to target specific subsets of nodes labeled with `lbpool.svccontroller.k3s.cattle.io/<pool-name>=true` or `svccontroller.k3s.cattle.io/lbpool=<pool-name>`. This is highly beneficial for edge deployments and multi-tenant clusters where you only want specific external-facing nodes to route incoming traffic.

### Dynamic Helm Controller & Failure Policy Upgrades ⚙️
To give operators finer control over embedded deployments, we have added the `--helm-controller-arg` flag, letting you pass custom flags directly to the embedded `helm-controller` process (cleanly replacing the deprecated `--helm-job-image` flag). Additionally, we have updated our packaged **Traefik** charts to default to a **`retry`** failure policy instead of a full **`reinstall`** during upgrades. This provides a vastly safer upgrade path by attempting to reconcile failures incrementally, preventing destructive resource deletion and recreation cycles on minor upgrade conflicts.
